            ...
            {recentActivityLoading ? (
                <div className="space-y-4">
                    {Array.from({ length: 2 }).map((_, idx) => (
                        <div key={idx} className="flex gap-4 p-4 bg-white border border-slate-100 rounded-xl animate-pulse">
                ...
                <div className="space-y-6">
                    {recentActivity.map((activity) => (
                        <div key={activity._id}>
                            ...
                            <div className="bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm hover:shadow-md hover:border-slate-300/80 transition-all duration-300">
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2.5 border-b border-slate-50">
                                ...
                                <div className="mt-3">
                                    ...
                                    >
                                        <span className="truncate">{activity.blog?.title || "View Post"}</span>
                ...
                <div className="w-full py-16 bg-white border border-slate-200/60 rounded-2xl text-center shadow-sm flex flex-col items-center justify-center p-6">
                    <div className="w-12 h-12 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100 text-slate-350 mb-3.5">
                        <FiMessageSquare size={20} />
                    </div>
                    <h3 className="font-bold text-slate-700 text-base">No Activity Found</h3>
                    <p className="text-xs text-slate-400 max-w-xs mt-1 balance">
